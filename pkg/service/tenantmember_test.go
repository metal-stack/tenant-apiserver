...
func TestCreateTenantMember(t *testing.T) {
	...
	tests := []struct {
		name    string
		prepare func()
		req     *v1.TenantMemberServiceCreateRequest
		want    *v1.TenantMemberServiceCreateResponse
		wantErr error
	}{
		{
			name: "create tenant member",
			req: &v1.TenantMemberServiceCreateRequest{
		...
		{
			name: "create tenant member without meta",
			req: &v1.TenantMemberServiceCreateRequest{
				TenantMember: &v1.TenantMember{
					Meta:      &v1.Meta{Id: "1"},
					TenantId:  "tenant-1",
		...
		},
		{
			...
			want:    nil,
			wantErr: errorutil.NotFound("unable to find tenant:%s for tenantMember", "unknown-tenant"),
		},
		{
			...
			want:    nil,
			wantErr: errorutil.NotFound("unable to find member:%s for tenantMember", "unknown-member"),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err == nil {
				assert.NotNil(t, got.TenantMember.Meta.CreatedTime)
			}
